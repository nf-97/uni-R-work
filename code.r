...
#stacked bar chart with both sets of data
cbPalette <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
...
#stats - fishers exact test to compare predicted stable age distributions
Predicted <- c("62.50%", "23.66%", "11.19%" , "2.65%", "0.00%")
Projected <- c("62.10%", "24.40%", "10.60%", "2.90%", "0.00%")
Age <- rep(c("0","1", "2", "3", "4"),2)
prop <- data.frame(Predicted, Projected, Age)

Combo <- c(Predicted, Projected)
labels <-  c(rep("Predicted Stable Age Distribution", 5), rep("Projected Stable Age Distribution", 5))
combo2 <- data.frame(Combo, labels, Age)
